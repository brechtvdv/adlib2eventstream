...
Adlib.prototype.fetchWithNTLMRecursively = async function(lastModifiedDate, startFrom, limit) {
    ...
    if (this._checkEuropeanaFlag || this._institutionName != "adlib" || lastModifiedDate) querypath += "&search=";
    else querypath += "&search=all";
    if (this._checkEuropeanaFlag) querypath += "webpublication=EUROPEANA";
    if (this._institutionName != "adlib") querypath += ` AND institution.name='${this._institutionName}'`;
