module.exports = {
    ...
    mapObjectCategorie: async (objectURI, input, mappedObject, adlib) => {
        ...
        if (input["object_category"] && input["object_category"][0]) {
            ...
            for (let c in input["object_category"]) {
                ...
                const categoryNameURI = await adlib.getURIFromPriref("thesaurus", input["object_category.lref"][c], "concept");
    ...
    mapVervaardiging: async (id, input, mappedObject, adlib) => {
        ...
        if(input.Production && input.Production[0]) {
                    ...
                    if (ontwerpRegex.test(note)) {
                        ...
                        for (let p in input["Production"]) {
                            ...
                            if (pro['creator.role'] && pro['creator.role'][0] === "ontwerper") {
                                const creatorURI = await adlib.getURIFromPriref("personen", input["object_category.lref"][c], "concept");
                                ...
                                c["@reverse"] = {
                                    "Rol.activiteit": {
                                        ...
                                        "Rol.rol": "http://vocab.getty.edu/aat/300386174"
                                    }
                                }
                            }
                        }
            ...
            for (let n in input["Production"]) {
                ...
                const creatorURI = await adlib.getURIFromPriref("personen", pro["creator.lref"][0], "concept");
                ...
                p["@reverse"] = {
                    "Rol.activiteit": {
                        "@type": "Rol",
                        "Rol.agent": creatorURI,
                        "Rol.rol": "http://vocab.getty.edu/aat/300197742"
                    }
                }
                productions.push(p);
            }
        ...
        if (input["Exhibition"]) {
            ...
            for (let e in input["Exhibition"]) {
                ...
                if (exhibition["exhibition.date.start"] && exhibition["exhibition.date.start"][0]) {
                    exh["Gebeurtenis.tijd"]["Periode.begin"] = exhibition["exhibition.date.start"][0];
                }
                if (exhibition["exhibition.date.end"] && exhibition["exhibition.date.end"][0]) {
                    exh["Gebeurtenis.tijd"]["Periode.einde"] = exhibition["exhibition.venue.place"][0];
                }
        ...
        if (input["Associated_person"] && input["Associated_person"][0]) {
            for (let p in input["Associated_person"]) {
                if (input["Associated_person"][p]["association.person"] && input["Associated_person"][p]["association.person"][0]) {
                    ...
                    const personURI = await adlib.getURIFromPriref("personen", input["Associated_person"][p]["association.person.lref"][0], "concept");
        ...
        if (input["Content_person"] && input["Content_person"][0]) {
            for (let p in input["Content_person"]) {
                if (input["Content_person"][p]["content.person.name"]) {
                    ...
                    const personURI = await adlib.getURIFromPriref("personen", input["Content_person"][p]["content.person.name.lref"][0], "concept");
    ...
    mapOpschriften: (input, mappedObject) => {
        let opschriften = [];
