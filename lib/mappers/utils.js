module.exports = {
    ...
    mapObjectnaam: (objectURI, input, mappedObject) => {
        let c = [];
    ...
    mapObjectCategorie: (objectURI, input, mappedObject) => {
    ...
    mapVerwervingDMG: (objectURI, institutionURI, input, mappedObject) => {
        ...
        mappedObject["MaterieelDing.isOvergedragenBijVerwerving"] = {
            "@type": "Verwerving",
            "Verwerving.overdrachtVan": objectURI,
    ...
    mapVerwervingStam: (objectURI, institutionURI, input, mappedObject) => {
        let datum = input["acquisition.date"] ? input["acquisition.date"][0] : "";
        let methode = input["acquisition.method"] ? input["acquisition.method"][0] : "";
    ...
    mapAssociaties: (objectURI, input, mappedObject) => {
        let c = [];
        ...
        if (input["Associated_subject"] && input["Associated_subject"][0]) {
            for (let p in input["Associated_subject"]) {
                if (input["Associated_subject"][p]["association.subject"] && input["Associated_subject"][p]["association.subject"][0]) {
                    let subjectLabel = input["Associated_subject"][p]["association.subject"][0];
        ...
        if (input["Associated_period"] && input["Associated_period"][0]) {
            for (let p in input["Associated_period"]) {
                if (input["Associated_period"][p]["association.period"] && input["Associated_period"][p]["association.period"][0]) {
                    let periodLabel = input["Associated_period"][p]["association.period"][0];
        ...
        if (input["Content_person"] && input["Content_person"][0]) {
            for (let p in input["Content_person"]) {
                if (input["Content_person"][p]["content.person.name"]) {
                    let person = input["Content_person"][p]["content.person.name"][0];
                    mappedObject["Entiteit.beeldtUit"] = {
                        label: person
                    };
                }
